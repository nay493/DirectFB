...
class DFBTestMultiLayerContext : public Insignia::Test
{
     INSIGNIA_TEST_CLASS( DFBTestMultiLayerContext );
...
public:
     struct ConfigMultiLayerContext : public Config {
          ConfigMultiLayerContext( unsigned int num_processes ) {
     ...
     virtual void GetConfigs( vector<Config*> &configs ) {
          for (unsigned int i=2; i<32; i++) {
               configs.push_back( new ConfigMultiLayerContext( i ));
          }
     }

     virtual void GetInfo( Info &info ) {
          info.name = "DFBTestMultiLayerContext";
     }
     ...
     virtual Result Run( const Config& configuration ) {
          const ConfigMultiLayerContext& config = dynamic_cast<const ConfigMultiLayerContext&>(configuration);
...
private:
     Result runMaster( const ConfigMultiLayerContext& config ) {
          receiveSlavesGo();
          postSlavesGo();
     ...
     Result runSlave( const ConfigMultiLayerContext& config ) {
          postMasterGo();
          receiveGo();
...
INSIGNIA_TEST_STATICS( DFBTestMultiLayerContext );
